        ...
        for y, line in enumerate(lines):
            for x, char in enumerate(line):
                if char in ["+", "-", "|", "/", "\\"]:
                    ...
                    self.cartPositions.add((x,y))
                elif char in ["^", "v"]:
                    self.cartPositions.add((x,y))
                    self.tiles[y][x] = ("|", Cart(char, (x,y)))

    def tick(self, number):
        for x, y in set(self.cartPositions):
            tile = self.tiles[y][x]

            if tile == None or tile[1] == None:
                continue
            ...
            self.cartPositions.remove((x,y))

            assert self.tiles[newY][newX] != None, f"{newX},{newY}"

            if self.tiles[newY][newX][1] != None:
                self.tiles[y][x] = (tile[0], None)
                self.tiles[newY][newX] = (self.tiles[newY][newX][0], None)
                self.cartPositions.remove((newX, newY))
            else:
                self.tiles[y][x] = (tile[0], None)
                newX, newY = newPosition
                self.tiles[newY][newX] = (self.tiles[newY][newX][0], cart)
                self.cartPositions.add((newX, newY))


        carts = len(self.cartPositions)

        print(f"\rCarts left: {carts} after {number} ticks   ", end="")

        if carts <= 1:
            (cpos,) = self.cartPositions
            return f"{cpos[0]},{cpos[1]}"
        else:
            return None
